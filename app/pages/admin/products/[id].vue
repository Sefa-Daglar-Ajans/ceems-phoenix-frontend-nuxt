...
async function handleSave() {
  clearErrors()
  const result = schema.safeParse({ ...form, price: Number(form.price), stock: Number(form.stock) })
  if (!result.success) {
  ...
  try {
    await updateProduct({ id: Number(productId.value), ...result.data })
        ...
        <form class="space-y-6" @submit.prevent="handleSave">
          <div class="flex items-center gap-4 pb-4 border-b border-gray-100">
            ...
            <div>
              ...
              <p class="text-sm text-gray-500">{{ product.sku }} — {{ product.series }}</p>
            </div>
          </div>
